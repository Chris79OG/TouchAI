...
describe('Settings Memory section', () => {
    ...
    it('uses memory wording, matching panel background, and toggles enabled state', async () => {
        ...
        expect(
            wrapper.get('[data-testid="settings-memory-content"]').attributes('style')
        ).toContain('background-color: rgb(251, 251, 246)');
    ...
    it('shows an empty memory state without old wording', async () => {
        vi.mocked(memoryQueries.findMemoryDirectoryItems).mockResolvedValue([]);
        ...
        expect(wrapper.text()).toContain('选择一条记忆');
        expect(wrapper.text()).not.toContain('长期记忆');
    });
});
